...
main (int argc, char *argv[])
{
	...
	filter = gst_element_factory_make("vertigotv", NULL);
	...
	if (gst_element_link (source, filter) != TRUE) {
		g_printerr ("failed to bind source->filter.\n");
		gst_object_unref (pipeline);
		return -1;
	}
	if(gst_element_link(filter, videoconvert) != TRUE)
	{
		g_printerr ("failed to bind filter->videoconvert.\n");
		gst_object_unref (pipeline);
		return -1;
	}
	if(gst_element_link(videoconvert, sink) != TRUE)
	{
		g_printerr ("failed to bind videoconvert->sink.\n");
		gst_object_unref (pipeline);
		return -1;
	}
